...
if ( ! class_exists( 'E_Register_Now__Tickets__Tickets' ) ) {
	...
	abstract class E_Register_Now__Tickets__Tickets {
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_e-rn-ticket-add-' . $this->className, array( $this, 'ajax_handler_ticket_add' ) );
			add_action( 'wp_ajax_e-rn-ticket-delete-' . $this->className, array( $this, 'ajax_handler_ticket_delete' ) );
			add_action( 'wp_ajax_e-rn-ticket-edit-' . $this->className, array( $this, 'ajax_handler_ticket_edit' ) );
			add_action( 'wp_ajax_e-rn-ticket-checkin-' . $this->className, array( $this, 'ajax_handler_attendee_checkin' ) );
			add_action( 'wp_ajax_e-rn-ticket-uncheckin-' . $this->className, array( $this, 'ajax_handler_attendee_uncheckin' ) );
		...
		public function getTemplateHierarchy( $template ) {
			...
			if ( $theme_file = locate_template( array( 'e-rn-events/' . $template ) ) ) {
				$file = $theme_file;
			} else {

 ...
 * List of RSVP Orders
 *
 * Override this template in your own theme by creating a file at [your-theme]/e-rn-events/tickets/orders-rsvp.php
 *
...
<div class="e-rn-rsvp">
	...
	<?php foreach ( $attendee_groups as $attendee_group ): ?>
		...
		<ul class="e-rn-rsvp-list e-rn-list">
			<?php foreach ( $attendee_group as $i => $attendee ): ?>
				<?php $key = $attendee['order_id']; ?>
				<li class="e-rn-item<?php echo $view->is_rsvp_restricted( $post_id, $attendee['product_id'] ) ? 'e-rn-disabled' : ''; ?>" <?php echo $view->get_restriction_attr( $post_id, $attendee['product_id'] ); ?> id="attendee-<?php echo $attendee['order_id']; ?>">
					<p class="list-attendee"><?php printf( esc_html__( 'Attendee %d', 'event-tickets' ), $i + 1 ); ?></p>
					<div class="e-rn-answer">

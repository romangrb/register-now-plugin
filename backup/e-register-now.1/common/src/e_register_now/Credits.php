...
class E_Register_Now__Credits {
	...
	public function html_comment_credit( $after_html ) {
		...
		$html_credit = "\n<!--\n" . esc_html__( 'This calendar is powered by %1$s.', 'e-rn-common' ) . "\nhttp://m.tri.be/18wn\n-->\n";
	...
	public function rating_nudge( $footer_text ) {
		...
		if ( $admin_helpers->is_screen() || $admin_helpers->is_post_type_screen() ) {
			...
			if ( class_exists( 'E_Register_Now__Events__Main' ) ) {
				...
				$footer_text = sprintf(
					esc_html__( 'Rate %1$sThe Events Calendar%2$s %3$s', 'e-rn-common' ),
					'<strong>',
					'</strong>',
				...
				$footer_text = sprintf(
					esc_html__( 'Rate %1$sEvent Tickets%2$s %3$s', 'e-rn-common' ),
					'<strong>',
					'</strong>',
					'<a href="' . $review_url . '" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a>'
				);
	...
	/**
	 * temporary function to filter event types down to only e-rn-specific types
	 *

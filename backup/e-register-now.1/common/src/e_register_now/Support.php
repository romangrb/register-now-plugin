...
if ( ! class_exists( 'E_Register_Now__Support' ) ) {
	...
	class E_Register_Now__Support {
		...
		public function getSupportStats() {
			...
			$keys = apply_filters( 'e-rn-pue-install-keys', array() );
			...
			$systeminfo = array(
				...
				'WordPress timezone' => get_option( 'timezone_string', esc_html__( 'Unknown or not set', 'e-rn-common' ) ),
				'server timezone'    => date_default_timezone_get(),
				'common library dir' => $GLOBALS['e-rn-common-info']['dir'],
				'common library version' => $GLOBALS['e-rn-common-info']['version'],
			);

			if ( $this->rewrite_rules_purged ) {
				$systeminfo['rewrite rules purged'] = esc_html__( 'Rewrite rules were purged on load of this help page. Chances are there is a rewrite rule flush occurring in a plugin or theme!', 'e-rn-common' );
			}

			$systeminfo = apply_filters( 'e-rn-events-pro-support', $systeminfo );

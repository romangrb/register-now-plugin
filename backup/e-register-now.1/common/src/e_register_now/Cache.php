...
class E_Register_Now__Cache {
	...
	public static function setup() {
		wp_cache_add_non_persistent_groups( array( 'e-rn-events-non-persistent' ) );
	}
	...
	public function set( $id, $value, $expiration = 0, $expiration_trigger = '' ) {
		if ( $expiration == self::NON_PERSISTENT ) {
			$group      = 'e-rn-events-non-persistent';
			$expiration = 1;
		} else {
			$group = 'e-rn-events';
		}
	...
	public function get( $id, $expiration_trigger = '' ) {
		return wp_cache_get( $this->get_id( $id, $expiration_trigger ), 'e-rn-events' );
	}
	...
	public function delete( $id, $expiration_trigger = '' ) {
		return wp_cache_delete( $this->get_id( $id, $expiration_trigger ), 'e-rn-events' );
	}

...
if ( ! class_exists( 'E_Register_Now__Field' ) ) {
	...
	class E_Register_Now__Field {
		...
		public function doField() {
			...
			if ( $this->conditional ) {
				...
				if ( $this->display_callback && is_callable( $this->display_callback ) ) {
					...
					// fail, log the error
					E_Register_Now__Main::debug( esc_html__( 'Invalid field type specified', 'e-rn-common' ), $this->type, 'notice' );
		...
		public function doFieldStart() {
			$return = '<fieldset id="e-rn-field-' . $this->id . '"';
			$return .= ' class="e-rn-field e-rn-field-' . $this->type;
			$return .= ( $this->error ) ? ' e-rn-error' : '';
			$return .= ( $this->size ) ? ' e-rn-size-' . $this->size : '';
		...
		public function doFieldLabel() {
			$return = '';
			if ( $this->label ) {
				$return = '<legend class="e-rn-field-label">' . $this->label . '</legend>';
			}
		...
		public function doFieldDivStart() {
			$return = '<div class="e-rn-field-wrap">';
		...
		public function radio() {
			...
			if ( is_array( $this->options ) ) {
				...
				$field .= '<span class="e-rn-error">' . esc_html__( 'No radio options specified', 'e-rn-common' ) . '</span>';
			}
			$field .= $this->doFieldDivEnd();
			$field .= $this->doFieldEnd();
		...
		public function checkbox_list() {
			...
			if ( is_array( $this->options ) ) {
				...
				$field .= '<span class="e-rn-error">' . esc_html__( 'No checkbox options specified', 'e-rn-common' ) . '</span>';
			}
			$field .= $this->doFieldDivEnd();
			$field .= $this->doFieldEnd();
		...
		public function dropdown() {
			...
			if ( is_array( $this->options ) && ! empty( $this->options ) ) {
				...
				$field .= '<span class="e-rn-error">' . esc_html__( 'No select options specified', 'e-rn-common' ) . '</span>';
			}
			$field .= $this->doFieldDivEnd();
			$field .= $this->doFieldEnd();

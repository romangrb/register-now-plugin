...
class Register_In_One_Click__Credits {
	...
	public function rating_nudge( $footer_text ) {
		...
		if ( $admin_helpers->is_screen() || $admin_helpers->is_post_type_screen() ) {
			...
			if ( class_exists( 'Register_In_One_Click__Events__Main' ) ) {
				...
				$footer_text = sprintf(
					esc_html__( 'Rate %1$sThe Events Calendar%2$s %3$s', 'rioc-common' ),
					'<strong>',
					'</strong>',
				...
				$footer_text = sprintf(
					esc_html__( 'Rate %1$sEvent Tickets%2$s %3$s', 'rioc-common' ),
					'<strong>',
					'</strong>',
					'<a href="' . $review_url . '" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a>'
				);
	...
	/**
	 * temporary function to filter event types down to only rioc-specific types
	 *

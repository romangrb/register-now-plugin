...
class Register_In_One_Click__Admin__Help_Page {
	...
	public function get_plugins( $plugin_name = null, $is_active = true ) {
		...
		$plugins['the-events-calendar'] = array(
			...
			'title' => esc_html__( 'The Events Calendar', 'rioc-common' ),
			...
			'description' => esc_html__( 'The Events Calendar is a carefully crafted, extensible plugin that lets you easily share your events.', 'rioc-common' ),
			'is_active' => false,
			'version' => null,
		);
		...
		$plugins['event-tickets'] = array(
			'name' => 'event-tickets',
			'title' => esc_html__( 'Event Tickets', 'rioc-common' ),
			...
			'description' => esc_html__( 'Events Tickets is a carefully crafted, extensible plugin that lets you easily sell tickets for your events.', 'rioc-common' ),
			'is_active' => false,
			'version' => null,
		);
		...
		$plugins['advanced-post-manager'] = array(
			...
			'title' => esc_html__( 'Advanced Post Manager', 'rioc-common' ),
			...
			'description' => esc_html__( 'Turbo charge your posts admin for any custom post type with sortable filters and columns, and auto-registration of metaboxes.', 'rioc-common' ),
			'is_active' => false,
			'version' => null,
		);
	...
	public function get_plugins_text( $is_active = true ) {
		...
		foreach ( $plugins as $plugin ) {
			...
			if ( $i === $count - 1 ) {
				$plugins_text .= esc_html__( ' and ', 'rioc-common' );
			} elseif ( $i !== $count ) {
				$plugins_text .= ', ';
			}
		}
	...
	private function get_plugin_api_data( $plugin = null ) {
		...
		if ( false === $data ) {
			...
			if ( ! is_wp_error( $data ) ) {
				...
				$data->downloaded = $data->downloaded ? number_format( $data->downloaded ) : _x( 'n/a', 'not available', 'rioc-common' );
			} else {
		...
		$data->up_to_date = ( version_compare( $plugin->version, $data->version, '<' ) ) ? esc_html__( 'You need to upgrade!', 'rioc-common' ) : esc_html__( 'You are up to date!', 'rioc-common' );
	...
	public function get_sections( $print = true ) {
		...
		foreach ( $sections as $index => $section ) {
			...
			$html[ $section->id . '-start' ] = '<div id="rioc-' . sanitize_html_class( $section->id ) . '" class="rioc-help-section clearfix rioc-section-type-' . sanitize_html_class( $section->type ) . '">';

			if ( ! empty( $section->title ) ) {
				$html[ $section->id . '-title' ] = '<h3 class="rioc-help-title">' . esc_html__( $section->title ) . '</h3>';
			}
	...
	public function print_plugin_box( $plugin ) {
		...
		if ( $api_data ) {
			...
			if ( 'install' !== $status['status'] && ! $plugin_active ) {
				...
				$link = '<a class="button" href="' . $activate_url . '" aria-label="' . esc_attr( sprintf( esc_attr__( 'Activate %s', 'rioc-common' ), $plugin->name ) ) . '">' . esc_html__( 'Activate Plugin', 'rioc-common' ) . '</a>';
				...
				$link = '<a class="button" href="' . $update_url . '">' . esc_html__( 'Upgrade Plugin', 'rioc-common' ) . '</a>';
			} elseif ( $plugin_exists ) {
				$link = '<a class="button disabled">' . esc_html__( 'You are up to date!', 'rioc-common' ) . '</a>';
			}
		}
		...
		if ( ! isset( $link ) ) {
			if ( $api_data ) {
				...
				$link = '<a class="button thickbox" href="' . $iframe_url . '" aria-label="' . esc_attr( sprintf( esc_attr__( 'Install %s', 'rioc-common' ), $plugin->name ) ) . '">' . esc_html__( 'Install Plugin', 'rioc-common' ) . '</a>';
			} else {
				$link = null;
			}
		}
		?>
		<div class="rioc-help-plugin-info">
			...
			<?php if ( $api_data ) { ?>
				<dl>
					<dt><?php esc_html_e( 'Latest Version:', 'rioc-common' ); ?></dt>
					<dd><?php echo esc_html( $api_data->version ); ?></dd>

					<dt><?php esc_html_e( 'Requires:', 'rioc-common' ); ?></dt>
					<dd><?php echo esc_html__( 'WordPress ', 'rioc-common' ) . esc_html( $api_data->requires ); ?>+</dd>

					<dt><?php esc_html_e( 'Active Users:', 'rioc-common' ); ?></dt>
					<dd><?php echo esc_html( number_format( $api_data->active_installs ) ); ?>+</dd>

					<dt><?php esc_html_e( 'Rating:', 'rioc-common' ); ?></dt>
			...
			<?php if ( ! empty( $addons ) ) { ?>
				<h3><?php esc_html_e( 'Premium Add-Ons', 'rioc-common' ); ?></h3>
				<ul class='rioc-list-addons'>
					<?php foreach ( $addons as $addon ) {
						$addon = (object) $addon;

						if ( isset( $addon->is_active ) && $addon->is_active ) {
							$active_title = __( 'Plugin Active', 'rioc-common' );
						} else {
							$active_title = __( 'Plugin Inactive', 'rioc-common' );
						}

						echo '<li title="' . esc_attr( $active_title ) . '" class="' . ( isset( $addon->is_active ) && $addon->is_active ? 'rioc-active-addon' : '' ) . '">';
						if ( isset( $addon->link ) ) {
							echo '<a href="' . esc_url( $addon->link ) . '" title="' . esc_attr__( 'Visit the Add-on Page', 'rioc-common' ) . '" target="_blank">';
						}

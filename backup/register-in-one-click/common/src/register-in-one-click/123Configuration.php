...
if ( ! class_exists( 'Register_In_One_Click__Configuration' ) ) {
	...
	class Register_In_One_Click__Configuration {
		 ...
		 */
		const MENU_SLUG  = 'rioc_common';
		const ADMIN_SLUG = 'rioc_common';
		...
		public function __construct() {
			...
			$this->errors      = get_option( 'rioc_settings_errors', array() );
		...
		// post_type=rioc&page=rioc_common
		// page=rioc_common&tab=general
		...
		public function add_configuration_page() {
			...
			$page_title = esc_html__( 'Configuration', 'rioc_common' );
			$menu_title = esc_html__( 'Configuration', 'rioc_common' );
		...
		public function generatePage() {
			...
			screen_icon();
			echo '<h1>';
			printf( esc_html__( '%s Settings', 'rioc_common' ), $this->menuName );
			echo '</h1>';
			...
			if ( ! has_action( 'rioc_settings_content_tab_' . $this->currentTab ) ) {
				echo '<p>' . esc_html__( "You've requested a non-existent tab.", 'rioc_common' ) . '</p>';
			}
			...
			if ( has_action( 'rioc_settings_content_tab_' . $this->currentTab ) && ! in_array( $this->currentTab, $this->noSaveTabs ) ) {
				...
				echo '<input id="tribeSaveSettings" class="button-primary" type="submit" name="tribeSaveSettings" value="' . esc_attr__( 'Save Changes', 'rioc_common' ) . '" />';
			}
		...
		public function validate() {
			...
			if ( isset( $_POST['tribeSaveSettings'] ) && isset( $_POST['current-settings-tab'] ) ) {
				...
				if ( ! current_user_can( 'manage_options' ) ) {
					$this->errors[]    = esc_html__( "You don't have permission to do that.", 'rioc_common' );
					$this->major_error = true;
				}

				// check the nonce
				if ( ! wp_verify_nonce( $_POST['rioc-save-settings'], 'saving' ) ) {
					$this->errors[]    = esc_html__( 'The request was sent insecurely.', 'rioc_common' );
					$this->major_error = true;
				}

				// check that the request originated from the current tab
				if ( $_POST['current-settings-tab'] != $this->currentTab ) {
					$this->errors[]    = esc_html__( "The request wasn't sent from this tab.", 'rioc_common' );
					$this->major_error = true;
				}
		...
		public function displayErrors() {
			...
			if ( apply_filters( 'rioc_settings_display_errors_or_not', ( $count > 0 ) ) ) {
				...
				$output .= esc_html__( 'Your form had the following errors:', 'rioc_common' );
				...
				if ( count( $errors ) ) {
					$message = ( isset( $this->major_error ) && $this->major_error )
						...
						: esc_html( _n( 'The above setting was not saved. Other settings were successfully saved.', 'The above settings were not saved. Other settings were successfully saved.', $count, 'rioc_common' ) );
				}
		...
		public function displaySuccess() {
			...
			if ( isset( $_GET['saved'] ) && ! apply_filters( 'rioc_settings_display_errors_or_not', ( $count > 0 ) ) ) {
				...
				$message = esc_html__( 'Settings saved.', 'rioc_common' );
		...
		public function add_toolbar_item() {
			...
			$wp_admin_bar->add_menu( array(
				...
				'title'  => esc_html__( 'Configuration', 'rioc_common' ),
		...
		public function do_menu_page() {
			...
			// include_once Register_In_One_Click__Main::instance()->plugin_path . 'src/admin-views/configuration.php';
		}

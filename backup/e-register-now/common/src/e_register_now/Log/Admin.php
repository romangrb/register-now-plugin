...
class E_Register_Now__Log__Admin {
	...
	protected function setup_script() {
		wp_enqueue_script( 'e-rn-common-logging-controls' );
		wp_localize_script( 'e-rn-common-logging-controls', 'e_rn_logger_data', array(
	...
	protected function get_available_logs() {
		...
		if ( empty( $available_logs ) ) {
			return array( '' => _x( 'None currently available', 'log selector', 'e-rn-common' ) );
		}
	...
	protected function get_log_engines(){
		...
		if ( empty( $available_engines ) ) {
			return array( '' => _x( 'None currently available', 'log engines', 'e-rn-common' ) );
		}
	...
	public function serve_log_downloads() {
		if ( empty( $_GET['e-rn-common-log'] ) || 'download' !== $_GET['e-rn-common-log'] ) {
			return;
		}
		...
		header( 'Content-Disposition: attachment; filename="e-rn-log-' . $log_name . '"' );

...
class E_Register_Now__Admin__Help_Page {
	...
	public function get_plugins( $plugin_name = null, $is_active = true ) {
		...
		$plugins['the-events-calendar'] = array(
			...
			'title' => esc_html__( 'The Events Calendar', 'e-rn-common' ),
			...
			'description' => esc_html__( 'The Events Calendar is a carefully crafted, extensible plugin that lets you easily share your events.', 'e-rn-common' ),
			'is_active' => false,
			'version' => null,
		);
		...
		$plugins['event-tickets'] = array(
			'name' => 'event-tickets',
			'title' => esc_html__( 'Event Tickets', 'e-rn-common' ),
			...
			'description' => esc_html__( 'Events Tickets is a carefully crafted, extensible plugin that lets you easily sell tickets for your events.', 'e-rn-common' ),
			'is_active' => false,
			'version' => null,
		);
		...
		$plugins['advanced-post-manager'] = array(
			...
			'title' => esc_html__( 'Advanced Post Manager', 'e-rn-common' ),
			...
			'description' => esc_html__( 'Turbo charge your posts admin for any custom post type with sortable filters and columns, and auto-registration of metaboxes.', 'e-rn-common' ),
			'is_active' => false,
			'version' => null,
		);
	...
	public function get_plugins_text( $is_active = true ) {
		...
		foreach ( $plugins as $plugin ) {
			...
			if ( $i === $count - 1 ) {
				$plugins_text .= esc_html__( ' and ', 'e-rn-common' );
			} elseif ( $i !== $count ) {
				$plugins_text .= ', ';
			}
		}
	...
	public function get_addons( $plugin = null, $is_active = null, $is_important = null ) {
		...
		$addons['events-calendar-pro'] = array(
			...
			'title' => esc_html__( 'Events Calendar PRO', 'e-rn-common' ),
		...
		$addons['eventbrite-tickets'] = array(
			...
			'title' => esc_html__( 'Eventbrite Tickets', 'e-rn-common' ),
		...
		$addons['community-events'] = array(
			'id' => 'community-events',
			'title' => esc_html__( 'Community Events', 'e-rn-common' ),
		...
		$addons['facebook-events'] = array(
			'id' => 'facebook-events',
			'title' => esc_html__( 'Facebook Events', 'e-rn-common' ),
		...
		$addons['events-filter-bar'] = array(
			...
			'title' => esc_html__( 'Events Filter Bar', 'e-rn-common' ),
		...
		$addons['event-tickets-plus'] = array(
			...
			'title' => esc_html__( 'Event Tickets Plus', 'e-rn-common' ),
		...
		$addons['event-community-tickets'] = array(
			...
			'title' => esc_html__( 'Community Tickets', 'e-rn-common' ),
	...
	private function get_plugin_api_data( $plugin = null ) {
		...
		if ( false === $data ) {
			...
			if ( ! is_wp_error( $data ) ) {
				...
				$data->downloaded = $data->downloaded ? number_format( $data->downloaded ) : _x( 'n/a', 'not available', 'e-rn-common' );
			} else {
		...
		$data->up_to_date = ( version_compare( $plugin->version, $data->version, '<' ) ) ? esc_html__( 'You need to upgrade!', 'e-rn-common' ) : esc_html__( 'You are up to date!', 'e-rn-common' );
	...
	public function get_sections( $print = true ) {
		...
		foreach ( $sections as $index => $section ) {
			...
			$html[ $section->id . '-start' ] = '<div id="e-rn-' . sanitize_html_class( $section->id ) . '" class="e-rn-help-section clearfix e-rn-section-type-' . sanitize_html_class( $section->type ) . '">';

			if ( ! empty( $section->title ) ) {
				$html[ $section->id . '-title' ] = '<h3 class="e-rn-help-title">' . esc_html__( $section->title ) . '</h3>';
			}
	...
	public function print_plugin_box( $plugin ) {
		...
		if ( $api_data ) {
			...
			if ( 'install' !== $status['status'] && ! $plugin_active ) {
				...
				$link = '<a class="button" href="' . $activate_url . '" aria-label="' . esc_attr( sprintf( esc_attr__( 'Activate %s', 'e-rn-common' ), $plugin->name ) ) . '">' . esc_html__( 'Activate Plugin', 'e-rn-common' ) . '</a>';
				...
				$link = '<a class="button" href="' . $update_url . '">' . esc_html__( 'Upgrade Plugin', 'e-rn-common' ) . '</a>';
			} elseif ( $plugin_exists ) {
				$link = '<a class="button disabled">' . esc_html__( 'You are up to date!', 'e-rn-common' ) . '</a>';
			}
		}
		...
		if ( ! isset( $link ) ) {
			if ( $api_data ) {
				...
				$link = '<a class="button thickbox" href="' . $iframe_url . '" aria-label="' . esc_attr( sprintf( esc_attr__( 'Install %s', 'e-rn-common' ), $plugin->name ) ) . '">' . esc_html__( 'Install Plugin', 'e-rn-common' ) . '</a>';
			} else {
				$link = null;
			}
		}
		?>
		<div class="e-rn-help-plugin-info">
			...
			<?php if ( $api_data ) { ?>
				<dl>
					<dt><?php esc_html_e( 'Latest Version:', 'e-rn-common' ); ?></dt>
					<dd><?php echo esc_html( $api_data->version ); ?></dd>

					<dt><?php esc_html_e( 'Requires:', 'e-rn-common' ); ?></dt>
					<dd><?php echo esc_html__( 'WordPress ', 'e-rn-common' ) . esc_html( $api_data->requires ); ?>+</dd>

					<dt><?php esc_html_e( 'Active Users:', 'e-rn-common' ); ?></dt>
					<dd><?php echo esc_html( number_format( $api_data->active_installs ) ); ?>+</dd>

					<dt><?php esc_html_e( 'Rating:', 'e-rn-common' ); ?></dt>
			...
			<?php if ( ! empty( $addons ) ) { ?>
				<h3><?php esc_html_e( 'Premium Add-Ons', 'e-rn-common' ); ?></h3>
				<ul class='e-rn-list-addons'>
					<?php foreach ( $addons as $addon ) {
						$addon = (object) $addon;

						if ( isset( $addon->is_active ) && $addon->is_active ) {
							$active_title = __( 'Plugin Active', 'e-rn-common' );
						} else {
							$active_title = __( 'Plugin Inactive', 'e-rn-common' );
						}

						echo '<li title="' . esc_attr( $active_title ) . '" class="' . ( isset( $addon->is_active ) && $addon->is_active ? 'e-rn-active-addon' : '' ) . '">';
						if ( isset( $addon->link ) ) {
							echo '<a href="' . esc_url( $addon->link ) . '" title="' . esc_attr__( 'Visit the Add-on Page', 'e-rn-common' ) . '" target="_blank">';
						}

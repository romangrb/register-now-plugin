...
if ( ! class_exists( 'E_Register_Now__PUE__Checker' ) ) {
	...
	class E_Register_Now__PUE__Checker {
		...
		public function hooks() {
			...
			add_filter( 'e-rn-pue-install-keys', array( $this, 'return_install_key' ) );
		}
		...
		public function do_license_key_fields( $fields ) {
			...
			$fields = self::array_insert_after_key( 'e-rn-form-content-start', $fields, array(
					...
					$this->pue_install_key => array(
						...
						'label'           => sprintf( esc_attr__( 'License Key', 'e-rn-common' ) ),
						'tooltip'         => esc_html__( 'A valid license key is required for support and updates', 'e-rn-common' ),
						'parent_option'   => false,
						'network_option'  => true,
					),
				)
		...
		public function do_license_key_javascript() {
			...
			<script>
				jQuery(document).ready(function ($) {
					$('#e-rn-field-<?php echo $this->pue_install_key ?>').change(function () {
						<?php echo $this->pue_install_key ?>_validateKey();
					});
					<?php echo $this->pue_install_key ?>_validateKey();
				});
				function <?php echo $this->pue_install_key ?>_validateKey() {
					var this_id       = '#e-rn-field-<?php echo $this->pue_install_key ?>';
		...
		public function do_license_key_success_message( $message, $tab ) {
			...
			return '<div id="message" class="updated"><p><strong>' . esc_html__( 'License key(s) updated.', 'e-rn-common' ) . '</strong></p></div>';
		...
		public function ajax_validate_key() {
			...
			if ( isset( $_POST['key'] ) ) {
				...
				$expiration = isset( $pluginInfo->expiration ) ? $pluginInfo->expiration : esc_html__( 'unknown date', 'e-rn-common' );

				if ( empty( $pluginInfo ) ) {
					$response['message'] = esc_html__( 'Sorry, key validation server is not available.', 'e-rn-common' );
					...
					if ( $api_secret_key && $api_secret_key === $queryArgs['pu_install_key'] ){
						$default_success_msg = sprintf( esc_html__( 'Valid Key! Expires on %s', 'e-rn-common' ), $expiration );
					} else {
						// Set the key
						update_option( $this->pue_install_key, $queryArgs['pu_install_key'] );

						$default_success_msg = sprintf( esc_html__( 'Thanks for setting up a valid key, it will expire on %s', 'e-rn-common' ), $expiration );
					}
				...
				$response['message'] = sprintf( esc_html__( 'Hmmm... something\'s wrong with this validator. Please contact %ssupport%s.', 'e-rn-common' ), '<a href="http://m.tri.be/1u">', '</a>' );
			}
		...
		private function get_api_message( $info ) {
			...
			$message = sprintf(
				esc_html__( 'Sorry, there is a problem with your license key. You\'ll need to %scheck your license%s to have access to updates, downloads, and support.', 'e-rn-common' ),

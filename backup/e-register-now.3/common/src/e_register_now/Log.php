...
class E_Register_Now__Log {
	...
	public function set_current_logger( $engine ) {
		...
		if ( ! isset( $available_engines[ $engine ] ) ) {
			throw new Exception( sprintf( __( 'Cannot set %s as the current logging engine', 'e-rn-common' ), $engine ) );
		}
	...
	public function get_logging_levels() {
		if ( empty( $this->levels ) ) {
			...
			$this->levels = (array) apply_filters( 'e_rn_common_logging_levels', array(
				array( self::DISABLE, __( 'Disabled', 'e-rn-common' ) ),
				array( self::ERROR,   __( 'Only errors', 'e-rn-common' ) ),
				array( self::WARNING, __( 'Warnings and errors', 'e-rn-common' ) ),
				array( self::DEBUG,   __( 'Full debug (all events)', 'e-rn-common' ) ),
			) );
		}

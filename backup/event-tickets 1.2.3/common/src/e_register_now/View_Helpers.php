...
if ( ! class_exists( 'E_Register_NowView_Helpers' ) ) {
	class E_Register_NowView_Helpers {
		...
		public static function is_24hr_format( $format = null ) {
			...
			$format = ( null === $format ) ? get_option( 'time_format', E_Register_NowDate_Utils::TIMEFORMAT ) : $format;
		...
		public static function getMeridianOptions( $date = '', $isStart = false ) {
			if ( strstr( get_option( 'time_format', E_Register_NowDate_Utils::TIMEFORMAT ), 'A' ) ) {
				$a         = 'A';

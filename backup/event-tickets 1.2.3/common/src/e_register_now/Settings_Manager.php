...
class E_Register_NowSettings_Manager {
	...
	public function init_options() {
		E_Register_NowSettings::instance();
	}
	...
	public function do_setting_tabs() {
		include_once E_Register_NowMain::instance()->plugin_path . 'src/admin-views/tribe-options-general.php';
		include_once E_Register_NowMain::instance()->plugin_path . 'src/admin-views/tribe-options-display.php';

		$showNetworkTabs = $this->get_network_option( 'showSettingsTabs', false );

		new E_Register_NowSettings_Tab( 'general', esc_html__( 'General', 'tribe-common' ), $generalTab );
		new E_Register_NowSettings_Tab( 'display', esc_html__( 'Display', 'tribe-common' ), $displayTab );

		$this->do_licenses_tab();

		new E_Register_NowSettings_Tab(
			'help',
	...
	public static function get_options() {
		$options = get_option( E_Register_NowMain::OPTIONNAME, array() );
		if ( has_filter( 'tribe_get_options' ) ) {
			_deprecated_function( 'tribe_get_options', '3.10', 'option_' . E_Register_NowMain::OPTIONNAME );
	...
	public static function set_options( $options, $apply_filters = true ) {
		...
		update_option( E_Register_NowMain::OPTIONNAME, $options );
	}
	...
	public static function get_network_options() {
		if ( ! isset( self::$network_options ) ) {
			$options              = get_site_option( E_Register_NowMain::OPTIONNAMENETWORK, array() );
	...
	public static function set_network_options( $options, $apply_filters = true ) {
		...
		if ( update_site_option( E_Register_NowMain::OPTIONNAMENETWORK, $options ) ) {
			self::$network_options = apply_filters( 'tribe_get_network_options', $options );
		} else {
	...
	public static function add_network_options_page() {
		$tribe_settings = E_Register_NowSettings::instance();
		add_submenu_page(
	...
	public static function do_network_settings_tab() {
		include_once E_Register_NowMain::instance()->plugin_path . 'src/admin-views/tribe-options-network.php';

		new E_Register_NowSettings_Tab( 'network', esc_html__( 'Network', 'tribe-common' ), $networkTab );
	}
	...
	protected function do_licenses_tab() {
		 ...
		 * @var $licenses_tab
		 */
		include E_Register_NowMain::instance()->plugin_path . 'src/admin-views/tribe-options-licenses.php';
		...
		new E_Register_NowSettings_Tab( 'licenses', esc_html__( 'Licenses', 'tribe-common' ), array(
			'priority'      => '40',
	...
	public function do_help_tab() {
		include_once E_Register_NowMain::instance()->plugin_path . 'src/admin-views/tribe-options-help.php';
	}
	...
	public function add_help_admin_menu_item() {
		...
		$parent = E_Register_NowSettings::$parent_slug;
		...
		$slug   = esc_url(
			apply_filters( 'tribe_settings_url',
				add_query_arg(
					...
					E_Register_NowSettings::$parent_page
				)
			)
		);
	...
	public function save_all_tabs_hidden() {
		...
		$network_options = (array) get_site_option( E_Register_NowMain::OPTIONNAMENETWORK );
	 ...
	 *
	 * @return E_Register_NowSettings_Manager
	 */

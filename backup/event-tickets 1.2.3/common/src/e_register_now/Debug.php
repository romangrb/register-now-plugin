...
class E_Register_NowDebug {
	...
	public static function render( $title, $data = false, $format = 'log' ) {
		...
		if ( E_Register_NowSettings_Manager::instance()->get_option( 'debugEvents' ) ) {
			$plugin = basename( dirname( E_Register_NowMain::instance()->plugin_path ) );

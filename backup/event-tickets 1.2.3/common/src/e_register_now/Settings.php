...
if ( ! class_exists( 'E_Register_NowSettings' ) ) {
	/**
	 * helper class that allows registration of settings
	 * this is a static class & uses the singleton design method
	 * instantiation takes place in E_Register_NowMain
	 *
	 */
	class E_Register_NowSettings {
		...
		/**
		 * @var E_Register_NowAdmin__Live_Date_Preview
		 */
		 ...
		 * @var E_Register_NowSettings|null
		 */
		...
		public function should_setup_pages() {
			if ( ! class_exists( 'E_Register_NowEvents__Main' ) ) {
				return true;
			}

			if ( version_compare( E_Register_NowEvents__Main::VERSION, '4.0beta', '>=' ) ) {
				return true;
			}
		...
		public function addPage() {
			...
			if ( ! is_multisite() || ( is_multisite() && '0' == E_Register_NowSettings_Manager::get_network_option( 'allSettingsTabsHidden', '0' ) ) ) {
		...
		public function initTabs() {
			if ( isset( $_GET['page'] ) && $_GET['page'] == $this->adminSlug ) {
				...
				$this->live_date_preview = new E_Register_NowAdmin__Live_Date_Preview;
				...
				if ( is_array( $fields ) ) {
					...
					foreach ( $fields as $field_id => $field ) {
						...
						if ( ( ! isset( $field['conditional'] ) || $field['conditional'] ) && ( ! empty( $field['validation_type'] ) || ! empty( $field['validation_callback'] ) ) ) {
							...
							// validate this sucka
							$validate = new E_Register_NowValidate( $field_id, $field, $value );
		...
		public function save() {
			...
			if ( ! empty( $this->validated ) ) {
				foreach ( $this->validated as $field_id => $validated_field ) {
					...
					if ( is_network_admin() ) {
						$parent_option = ( isset( $validated_field->field['parent_option'] ) ) ? $validated_field->field['parent_option'] : E_Register_NowMain::OPTIONNAMENETWORK;
					}
					if ( ! is_network_admin() ) {
						$parent_option = ( isset( $validated_field->field['parent_option'] ) ) ? $validated_field->field['parent_option'] : E_Register_NowMain::OPTIONNAME;
					}
			 ...
			 * this will save using the E_Register_NowSettings_Manager::set_options method.
			 */
			foreach ( $parent_options as $option_id => $new_options ) {
				// get the old options
				if ( $option_id == E_Register_NowMain::OPTIONNAME ) {
				...
				if ( $option_id == E_Register_NowMain::OPTIONNAME ) {
					// save using the E_Register_NowSettings_Manager method
					E_Register_NowSettings_Manager::set_options( $options );
				} elseif ( $option_id == E_Register_NowMain::OPTIONNAMENETWORK ) {
					E_Register_NowSettings_Manager::set_network_options( $options );
				} else {

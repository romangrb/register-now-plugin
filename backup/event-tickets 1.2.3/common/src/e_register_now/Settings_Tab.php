...
if ( ! class_exists( 'E_Register_NowSettings_Tab' ) ) {
	/**
	 ...
	 *
	 */
	class E_Register_NowSettings_Tab {
		...
		/**
		 * filters the tabs array from E_Register_NowSettings
		 * and adds the current tab to it
		 * does not add a tab if it's empty
		 *
		 * @param array $tabs the $tabs from E_Register_NowSettings
		 *
		 * @return array $tabs the filtered tabs
		 */
		public function addTab( $tabs ) {
			$hideSettingsTabs = E_Register_NowSettings_Manager::get_network_option( 'hideSettingsTabs', array() );
		 ...
		 *
		 * @param array $allTabs All the tabs from E_Register_NowSettings.
		 *
		...
		/**
		 * filters the fields array from E_Register_NowSettings
		 * and adds the current tab's fields to it
		 *
		 * @param array $field the $fields from E_Register_NowSettings
		 *
		 ...
		 * button or not
		 *
		 * @param array $noSaveTabs the $noSaveTabs from E_Register_NowSettings
		 *
			...
			if ( is_array( $this->fields ) && ! empty( $this->fields ) ) {
				foreach ( $this->fields as $key => $field ) {
					if ( isset( $sent_data[ $key ] ) ) {
						...
						if ( is_network_admin() ) {
							$parent_option = ( isset( $field['parent_option'] ) ) ? $field['parent_option'] : E_Register_NowMain::OPTIONNAMENETWORK;
						}
						if ( ! is_network_admin() ) {
							$parent_option = ( isset( $field['parent_option'] ) ) ? $field['parent_option'] : E_Register_NowMain::OPTIONNAME;
						}
						...
						if ( ! $parent_option ) {
							...
							if ( $parent_option == E_Register_NowMain::OPTIONNAME ) {
								// get the options from E_Register_NowSettings_Manager if we're getting the main array
								$value = E_Register_NowSettings_Manager::get_option( $key, $default );
							} elseif ( $parent_option == E_Register_NowMain::OPTIONNAMENETWORK ) {
								$value = E_Register_NowSettings_Manager::get_network_option( $key, $default );
							} else {
					...
					// create the field
					new E_Register_NowField( $key, $field, $value );
				}
			} else {

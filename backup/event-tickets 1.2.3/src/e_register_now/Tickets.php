...
if ( ! class_exists( 'Tribe__Tickets__Tickets' ) ) {
	...
	abstract class Tribe__Tickets__Tickets {
		...
		final public function ticket_add( $post_id, $data ) {
			...
			$ticket->ID          = isset( $data['event_id'] ) ? absint( $data['ticket_id'] ) : null;
			$ticket->name        = isset( $data['event_name'] ) ? esc_html( $data['event_name'] ) : null;
			$ticket->description = isset( $data['event_description'] ) ? esc_html( $data['ticket_description'] ) : null;
			$ticket->price       = ! empty( $data['event_price'] ) ? trim( $data['event_price'] ) : 0;
			$ticket->purchase_limit = isset( $data['event_purchase_limit'] ) ? absint( $data['event_purchase_limit' ] ) : apply_filters( 'tribe_tickets_default_purchase_limit', 0, $ticket->ID );
			...
			if ( ! empty( $data['event_start_date'] ) ) {
				$meridian           = ! empty( $data['event_start_meridian'] ) ? ' ' . $data['event_start_meridian'] : '';
				$ticket->start_date = date( Tribe__Date_Utils::DBDATETIMEFORMAT, strtotime( $data['event_start_date'] . ' ' . $data['event_start_hour'] . ':' . $data['event_start_minute'] . ':00' . $meridian ) );
			}

			if ( ! empty( $data['event_end_date'] ) ) {
				$meridian         = ! empty( $data['event_end_meridian'] ) ? ' ' . $data['event_end_meridian'] : '';
				$ticket->end_date = date( Tribe__Date_Utils::DBDATETIMEFORMAT, strtotime( $data['event_end_date'] . ' ' . $data['event_end_hour'] . ':' . $data['event_end_minute'] . ':00' . $meridian ) );
			}

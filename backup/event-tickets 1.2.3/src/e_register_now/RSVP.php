...
class Tribe__Tickets__RSVP extends Tribe__Tickets__Tickets {
	 ...
	 * @var string
	 */
	public $rn_event_object = 'register_now_event';

	/**
	 * Meta key that relates Events in callendar
	 * @var string
	 */
	public $event_key = '_key_for_event';
	...
	public function register_types() {
		...
		register_post_type( $this->rn_event_object, array(
			'label'           => 'Tickets',
			'public'          => false,
			'show_ui'         => false,
			'show_in_menu'    => false,
	...
	protected function get_attendees_by_transaction( $order_id ) {
		...
		foreach ( $query->posts as $post ) {
			...
			$attendees[] = array(
				...
				'event_sent'    => (bool) get_post_meta( $post->ID, self::ATTENDEE_TICKET_SENT, true ),
			);
		}
	...
	public function save_ticket( $event_id, $ticket, $raw_data = array() ) {
		...
		if ( empty( $ticket->ID ) ) {
			...
			$args = array(
				'post_status'  => 'publish',
				'post_type'    => $this->rn_event_object,

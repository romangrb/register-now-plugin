 ...
 * List of RSVP Orders
 *
 * Override this template in your own theme by creating a file at [your-theme]/rioc-events/tickets/orders-rsvp.php
 *
...
<div class="rioc-rsvp">
	...
	<?php foreach ( $attendee_groups as $attendee_group ): ?>
		...
		<ul class="rioc-rsvp-list rioc-list">
			<?php foreach ( $attendee_group as $i => $attendee ): ?>
				<?php $key = $attendee['order_id']; ?>
				<li class="rioc-item<?php echo $view->is_rsvp_restricted( $post_id, $attendee['product_id'] ) ? 'rioc-disabled' : ''; ?>" <?php echo $view->get_restriction_attr( $post_id, $attendee['product_id'] ); ?> id="attendee-<?php echo $attendee['order_id']; ?>">
					<p class="list-attendee"><?php printf( esc_html__( 'Attendee %d', 'event-tickets' ), $i + 1 ); ?></p>
					<div class="rioc-answer">

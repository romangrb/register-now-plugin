...
if ( ! class_exists( 'Register_In_One_Click__Tickets__Tickets' ) ) {
	...
	abstract class Register_In_One_Click__Tickets__Tickets {
		...
		final public function ticket_add( $post_id, $data ) {
			...
			$ticket->primary_key    = isset( $data['ticket_id'] ) ? absint( $data['ticket_id'] ) : null; 
			...
			$ticket->event_code = isset( $data['event_code'] ) ? absint( $data['event_code'] ) : null;
			$ticket->event_category = isset( $data['event_category'] ) ? absint( $data['event_category'] ) : null;
			...
			$ticket->message3	    = isset( $data['message2'] ) ? esc_html( $data['message2'] ) : null;
			...
			if ( ! empty( $data['reg_period_start_date'] ) ) {
				...
				$ticket->start_date = date( Register_In_One_Click__Date_Utils::DBDATETIMEFORMAT, strtotime( $data['ticket_start_date'] . ' ' . $data['reg_period_start_hour'] . ':' . $data['reg_period_start_minute'] . ':00' . $meridian ) );
			}

			if ( ! empty( $data['reg_period_end_date'] ) ) {
				$meridian         = ! empty( $data['reg_period_end_meridian'] ) ? ' ' . $data['reg_period_end_meridian'] : '';
				$ticket->end_date = date( Register_In_One_Click__Date_Utils::DBDATETIMEFORMAT, strtotime( $data['ticket_end_date'] . ' ' . $data['reg_period_end_hour'] . ':' . $data['reg_period_end_minute'] . ':00' . $meridian ) );
			}
		...
		final public function ajax_handler_ticket_edit() {
			...
			$return['message3'] = html_entity_decode( $return['message2'], ENT_QUOTES );
			$return['message3'] = htmlspecialchars_decode( $return['message2'] );

...
if ( ! class_exists( 'Register_In_One_Click__Tickets__Tickets' ) ) {
	...
	abstract class Register_In_One_Click__Tickets__Tickets {
			...
			*/
			...
			if ( $return ) {
				...
				$return = (! $is_required_data_empty) ? 
					$this->notice( esc_html__( 'Your event has been saved.', 'event-tickets' ) ) . $return :
					$this->notice( esc_html__( 'The event has been saved as draft (some of required fields were autogenerated by prefix - auto_ ... , you can change them). Please fill all required fields in form', 'event-tickets' ) . $return, 'update-nag');
				/**
		...
		protected function notice_err( $msg ) {
			return sprintf( '<div class="wrap"><div class="error"><p>%s</p></div></div>', $msg );
		}

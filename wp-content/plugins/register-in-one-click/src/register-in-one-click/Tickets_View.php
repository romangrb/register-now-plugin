...
class Register_In_One_Click__Tickets__Tickets_View {
	...
	public function prevent_page_redirect( $query ) {
		$is_correct_page = isset( $query->query_vars['rioc-edit-orders'] ) && $query->query_vars['rioc-edit-orders'];
	...
	public function rewrite_rules_array() {
		...
		$rules = array(
			sanitize_title_with_dashes( $bases['tickets'][0] ) . '/([0-9]{1,})/?' => 'index.php?p=$matches[1]&rioc-edit-orders=1',
		);
		return $rules;
	}
	...
	public function update_tickets() {
		$is_correct_page = get_query_var( 'rioc-edit-orders', false );
		...
		if ( empty( $_POST['process-tickets'] ) || ( empty( $_POST['attendee'] ) && empty( $_POST['rioc-tickets-meta'] ) ) ) {
			return;
		}

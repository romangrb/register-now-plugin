...
<form method="POST" class="rioc-attendees-email">
	...
	<div id="attendees_email_wrapper">
		...
		<?php
		if ( false !== $status ) {
			echo '<div class="rioc-attendees-email-message ' . ( is_wp_error( $status ) ? 'error ' : 'updated ' ) . 'notice is-dismissible">';
		...
		<?php echo '<button type="submit" class="button button-primary right" name="rioc-send-email" value="1">' . esc_html__( 'Send Email', 'event-tickets' ) . '</button>'; ?>
	</div>
</form>
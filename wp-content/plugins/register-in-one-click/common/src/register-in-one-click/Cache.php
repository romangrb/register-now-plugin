...
class E_Register_Now__Cache {
	...
	public static function setup() {
		wp_cache_add_non_persistent_groups( array( 'rioc-events-non-persistent' ) );
	}
	...
	public function set( $id, $value, $expiration = 0, $expiration_trigger = '' ) {
		if ( $expiration == self::NON_PERSISTENT ) {
			$group      = 'rioc-events-non-persistent';
			$expiration = 1;
		} else {
			$group = 'rioc-events';
		}
	...
	public function get( $id, $expiration_trigger = '' ) {
		return wp_cache_get( $this->get_id( $id, $expiration_trigger ), 'rioc-events' );
	}
	...
	public function delete( $id, $expiration_trigger = '' ) {
		return wp_cache_delete( $this->get_id( $id, $expiration_trigger ), 'rioc-events' );
	}

...
if ( ! class_exists( 'E_Register_Now__Support' ) ) {
	...
	class E_Register_Now__Support {
		...
		public function getSupportStats() {
			...
			$keys = apply_filters( 'rioc-pue-install-keys', array() );
			...
			$systeminfo = array(
				...
				'common library version' => $GLOBALS['rioc-common-info']['version'],
			);

			if ( $this->rewrite_rules_purged ) {
				$systeminfo['rewrite rules purged'] = esc_html__( 'Rewrite rules were purged on load of this help page. Chances are there is a rewrite rule flush occurring in a plugin or theme!', 'rioc-common' );
			}

			$systeminfo = apply_filters( 'rioc-events-pro-support', $systeminfo );

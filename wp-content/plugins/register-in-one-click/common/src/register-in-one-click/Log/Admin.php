...
class Register_In_One_Click__Log__Admin {
	...
	protected function get_available_logs() {
		...
		if ( empty( $available_logs ) ) {
			return array( '' => _x( 'None currently available', 'log selector', 'rioc-common' ) );
		}
	...
	protected function get_log_engines(){
		...
		if ( empty( $available_engines ) ) {
			return array( '' => _x( 'None currently available', 'log engines', 'rioc-common' ) );
		}
	...
	public function serve_log_downloads() {
		if ( empty( $_GET['rioc-common-log'] ) || 'download' !== $_GET['rioc-common-log'] ) {
			return;
		}

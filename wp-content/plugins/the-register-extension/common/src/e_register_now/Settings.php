...
if ( ! class_exists( 'E__Register__Now__Settings' ) ) {
	/**
	 * helper class that allows registration of settings
	 * this is a static class & uses the singleton design method
	 * instantiation takes place in E__Register__Now__Main
	 *
	 */
	class E__Register__Now__Settings {

		/**
		 * Slug of the parent menu slug
		 * @var string
		 */
		public static $parent_slug = 'ern-common';
		...
		/**
		 * @var E__Register__Now__Admin__Live_Date_Preview
		 */
		 ...
		 * @var E__Register__Now__Settings|null
		 */
		 ...
		 *
		 * @return E__Register__Now__Settings
		 */
		...
		public function __construct() {
			...
			$this->menuName    = apply_filters( 'tribe_settings_menu_name', esc_html__( 'Events', 'ern-common' ) );
			$this->requiredCap = apply_filters( 'tribe_settings_req_cap', 'manage_options' );
			$this->adminSlug   = apply_filters( 'tribe_settings_admin_slug', 'ern-common' );
		 ...
		 * that version of TEC (and lower) is ern-common ignorant. Therefore, ern-common has to be
		...
		public function should_setup_pages() {
			...
			if ( version_compare( E__Register__Now__Events__Main::VERSION, '4.0beta', '>=' ) ) {
				return true;
			}
		...
		public function addPage() {
			...
			if ( ! is_multisite() || ( is_multisite() && '0' == E__Register__Now__Settings_Manager::get_network_option( 'allSettingsTabsHidden', '0' ) ) ) {
				if ( post_type_exists( 'tribe_events' ) ) {
					self::$parent_page = 'edit.php?post_type=tribe_events';
				} else {
					self::$parent_page = 'admin.php?page=ern-common';

					add_menu_page(
						esc_html__( 'Events', 'ern-common' ),
						esc_html__( 'Events', 'ern-common' ),
				...
				$this->admin_page = add_submenu_page(
					...
					esc_html__( 'Events Settings', 'ern-common' ),
					esc_html__( 'Settings', 'ern-common' ),
					$this->requiredCap,
					self::$parent_slug,
		...
		public function addNetworkPage() {
			...
			$this->admin_page = add_submenu_page(
				'settings.php', esc_html__( 'Events Settings', 'ern-common' ), esc_html__( 'Events Settings', 'ern-common' ), $this->requiredCap, $this->adminSlug, array(
					$this,
					'generatePage',
				)
			);
		...
		public function initTabs() {
			if ( isset( $_GET['page'] ) && $_GET['page'] == $this->adminSlug ) {
				...
				$this->live_date_preview = new E__Register__Now__Admin__Live_Date_Preview;
		...
		public function generatePage() {
			...
			screen_icon();
			echo '<h1>';
			printf( esc_html__( '%s Settings', 'ern-common' ), $this->menuName );
			echo '</h1>';
			...
			echo '<div class="ern-settings-form form">';
			...
			if ( ! has_action( 'tribe_settings_content_tab_' . $this->currentTab ) ) {
				echo '<p>' . esc_html__( "You've requested a non-existent tab.", 'ern-common' ) . '</p>';
			}
			...
			if ( has_action( 'tribe_settings_content_tab_' . $this->currentTab ) && ! in_array( $this->currentTab, $this->noSaveTabs ) ) {
				wp_nonce_field( 'saving', 'ern-save-settings' );
				echo '<div class="clear"></div>';
				echo '<input type="hidden" name="current-settings-tab" id="current-settings-tab" value="' . esc_attr( $this->currentTab ) . '" />';
				echo '<input id="tribeSaveSettings" class="button-primary" type="submit" name="tribeSaveSettings" value="' . esc_attr__( 'Save Changes', 'ern-common' ) . '" />';
			}
		...
		public function generateTabs() {
			if ( is_array( $this->tabs ) && ! empty( $this->tabs ) ) {
				echo '<h2 id="ern-settings-tabs" class="nav-tab-wrapper">';
		...
		public function validate() {
			...
			if ( isset( $_POST['tribeSaveSettings'] ) && isset( $_POST['current-settings-tab'] ) ) {
				...
				if ( ! current_user_can( 'manage_options' ) ) {
					$this->errors[]    = esc_html__( "You don't have permission to do that.", 'ern-common' );
					$this->major_error = true;
				}

				// check the nonce
				if ( ! wp_verify_nonce( $_POST['ern-save-settings'], 'saving' ) ) {
					$this->errors[]    = esc_html__( 'The request was sent insecurely.', 'ern-common' );
					$this->major_error = true;
				}

				// check that the request originated from the current tab
				if ( $_POST['current-settings-tab'] != $this->currentTab ) {
					$this->errors[]    = esc_html__( "The request wasn't sent from this tab.", 'ern-common' );
					$this->major_error = true;
				}
				...
				if ( is_array( $fields ) ) {
					...
					foreach ( $fields as $field_id => $field ) {
						...
						if ( ( ! isset( $field['conditional'] ) || $field['conditional'] ) && ( ! empty( $field['validation_type'] ) || ! empty( $field['validation_callback'] ) ) ) {
							...
							// validate this sucka
							$validate = new E__Register__Now__Validate( $field_id, $field, $value );
		...
		public function save() {
			...
			if ( ! empty( $this->validated ) ) {
				foreach ( $this->validated as $field_id => $validated_field ) {
					...
					if ( is_network_admin() ) {
						$parent_option = ( isset( $validated_field->field['parent_option'] ) ) ? $validated_field->field['parent_option'] : E__Register__Now__Main::OPTIONNAMENETWORK;
					}
					if ( ! is_network_admin() ) {
						$parent_option = ( isset( $validated_field->field['parent_option'] ) ) ? $validated_field->field['parent_option'] : E__Register__Now__Main::OPTIONNAME;
					}
			 ...
			 * this will save using the E__Register__Now__Settings_Manager::set_options method.
			 */
			foreach ( $parent_options as $option_id => $new_options ) {
				// get the old options
				if ( $option_id == E__Register__Now__Main::OPTIONNAME ) {
				...
				if ( $option_id == E__Register__Now__Main::OPTIONNAME ) {
					// save using the E__Register__Now__Settings_Manager method
					E__Register__Now__Settings_Manager::set_options( $options );
				} elseif ( $option_id == E__Register__Now__Main::OPTIONNAMENETWORK ) {
					E__Register__Now__Settings_Manager::set_network_options( $options );
				} else {
		...
		public function displayErrors() {
			...
			if ( apply_filters( 'tribe_settings_display_errors_or_not', ( $count > 0 ) ) ) {
				...
				$output .= esc_html__( 'Your form had the following errors:', 'ern-common' );
				$output .= '</strong></p><ul class="ern-errors-list">';
				...
				if ( count( $errors ) ) {
					$message = ( isset( $this->major_error ) && $this->major_error )
						...
						: esc_html( _n( 'The above setting was not saved. Other settings were successfully saved.', 'The above settings were not saved. Other settings were successfully saved.', $count, 'ern-common' ) );
				}
		...
		public function displaySuccess() {
			...
			if ( isset( $_GET['saved'] ) && ! apply_filters( 'tribe_settings_display_errors_or_not', ( $count > 0 ) ) ) {
				...
				$message = esc_html__( 'Settings saved.', 'ern-common' );
		...
		public function get_parent_slug() {
			...
			// if we don't have an event post type, then we can just use the ern-common slug
			if ( 'edit.php' === $slug || 'admin.php?page=ern-common' === $slug ) {

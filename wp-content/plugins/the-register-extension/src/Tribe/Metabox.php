...
class Tribe__Tickets__Metabox {
	...
	public static function maybe_add_meta_box( $post_type ) {
		...
		if ( ! in_array( $post_type, E__Register__Now::instance()->post_types() ) ) {
			return;
		}
	...
	public static function add_admin_scripts( $hook ) {
		...
		if ( ( $hook != 'post-new.php' && $hook != 'post.php' ) || ! in_array( $post->post_type, E__Register__Now::instance()->post_types() ) || empty( $modules ) ) {
			return;
		}

		$resources_url = plugins_url( 'src/resources', dirname( dirname( __FILE__ ) ) );

		wp_enqueue_style( 'event-tickets', $resources_url .'/css/tickets.css', array(), E__Register__Now::instance()->css_version() );
		wp_enqueue_script( 'event-tickets', $resources_url .'/js/tickets.js', array( 'jquery-ui-datepicker' ), E__Register__Now::instance()->js_version(), true );

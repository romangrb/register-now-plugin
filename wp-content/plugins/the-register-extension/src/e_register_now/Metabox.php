 ...
 *     available.
 */
class E__Register__Now__Tickets__Metabox {
	...
	public static function maybe_add_meta_box( $post_type ) {
		...
		if ( ! in_array( $post_type, E__Register__Now__Tickets__Main::instance()->post_types() ) ) {
			return;
		}
		...
		add_meta_box(
			...
			array(
				'E__Register__Now__Tickets__Metabox',
	...
	public static function do_modules_metaboxes( $post_id ) {
		...
		E__Register__Now__Tickets__Tickets_Handler::instance()->do_meta_box( $post_id );
	}
	...
	public static function add_admin_scripts( $hook ) {
		...
		if ( ( $hook != 'post-new.php' && $hook != 'post.php' ) || ! in_array( $post->post_type, E__Register__Now__Tickets__Main::instance()->post_types() ) || empty( $modules ) ) {
			return;
		}

		$resources_url = plugins_url( 'src/resources', dirname( dirname( __FILE__ ) ) );

		wp_enqueue_style( 'event-tickets', $resources_url .'/css/tickets.css', array(), E__Register__Now__Tickets__Main::instance()->css_version() );
		wp_enqueue_script( 'event-tickets', $resources_url .'/js/tickets.js', array( 'jquery-ui-datepicker' ), E__Register__Now__Tickets__Main::instance()->js_version(), true );

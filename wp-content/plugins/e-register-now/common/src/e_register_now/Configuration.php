...
if ( ! class_exists( 'E_Register_Now__Configuration' ) ) {
	...
	class E_Register_Now__Configuration {
		...
		public function generatePage() {
			...
			screen_icon();
			echo '<h1>';
			printf( esc_html__( '%s Settings', 'e-rn-common' ), $this->menuName );
			echo '</h1>';
			...
			if ( ! has_action( 'e_rn_settings_content_tab_' . $this->currentTab ) ) {
				echo '<p>' . esc_html__( "You've requested a non-existent tab.", 'e-rn-common' ) . '</p>';
			}
			...
			if ( has_action( 'e_rn_settings_content_tab_' . $this->currentTab ) && ! in_array( $this->currentTab, $this->noSaveTabs ) ) {
				...
				echo '<input id="tribeSaveSettings" class="button-primary" type="submit" name="tribeSaveSettings" value="' . esc_attr__( 'Save Changes', 'e-rn-common' ) . '" />';
			}
		...
		public function validate() {
			...
			if ( isset( $_POST['tribeSaveSettings'] ) && isset( $_POST['current-settings-tab'] ) ) {
				...
				if ( ! current_user_can( 'manage_options' ) ) {
					$this->errors[]    = esc_html__( "You don't have permission to do that.", 'e-rn-common' );
					$this->major_error = true;
				}

				// check the nonce
				if ( ! wp_verify_nonce( $_POST['e-rn-save-settings'], 'saving' ) ) {
					$this->errors[]    = esc_html__( 'The request was sent insecurely.', 'e-rn-common' );
					$this->major_error = true;
				}

				// check that the request originated from the current tab
				if ( $_POST['current-settings-tab'] != $this->currentTab ) {
					$this->errors[]    = esc_html__( "The request wasn't sent from this tab.", 'e-rn-common' );
					$this->major_error = true;
				}
		...
		public function displayErrors() {
			...
			if ( apply_filters( 'e_rn_settings_display_errors_or_not', ( $count > 0 ) ) ) {
				...
				$output .= esc_html__( 'Your form had the following errors:', 'e-rn-common' );
				...
				if ( count( $errors ) ) {
					$message = ( isset( $this->major_error ) && $this->major_error )
						...
						: esc_html( _n( 'The above setting was not saved. Other settings were successfully saved.', 'The above settings were not saved. Other settings were successfully saved.', $count, 'e-rn-common' ) );
				}

...
(function( window, $, undefined ) {
	...
	$( document ).ready( function() {
		var $event_pickers = $( '#e-rn-event-datepickers' ),
			$e_rn_tickets = $( '#tribetickets' ),
			$tickets_container = $( '#event_tickets' ),
			$enable_global_stock = $( "#e-rn-tickets-enable-global-stock" ),
			$global_stock_level = $( "#e-rn-tickets-global-stock-level" ),
		...
		$e_rn_tickets.on( {
			...
			'spin.tribe': function( event, action ) {
				...
				if ( 'stop' === action ) {
					$tickets_container.css( 'opacity', '1' )
						.find( '#e-rn-loading' ).hide();
				} else {
					$tickets_container.css( 'opacity', '0.5' )
						.find( '#e-rn-loading' ).show();
			...
			'clear.tribe': function() {
				...
				$('#e-rn-tickets-attendee-sortables').empty();
				$('.e-rn-tickets-attendee-saved-fields').show();
		...
		$( '#ticket_form_save' ).click( function( e ) {
			...
			var params = {
				action  : 'e-rn-ticket-add-' + $( 'input[name=ticket_provider]:checked' ).val(),
		...
		$e_rn_tickets.on( 'click', '.ticket_delete', function( e ) {
			...
			var params = {
				action   : 'e-rn-ticket-delete-' + $( this ).attr( 'attr-provider' ),
				post_ID  : $( '#post_ID' ).val(),
		...
		$e_rn_tickets
			.on( 'click', '.ticket_edit', function( e ) {
				...
				var params = {
					action   : 'e-rn-ticket-edit-' + $( this ).attr( 'attr-provider' ),
					post_ID  : $( '#post_ID' ).val(),
					ticket_id: $( this ).attr( 'attr-ticket-id' ),
